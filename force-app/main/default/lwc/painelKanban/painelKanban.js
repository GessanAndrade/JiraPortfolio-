...
export default class PainelKanban extends LightningElement {
    ...
    handleDrop(event) {
        ...
        if (this.origemColuna === destinoColuna) {
            console.log(`"${this.draggedCardTitle}" ficou em "${destinoColuna}" (não mudou de coluna)`);
            return;
        }
        ...
        console.log(`"${this.draggedCardId}" foi movida de "${this.origemColuna}" para "${destinoColuna}"`);

        updateTaskStatus({ taskId: this.draggedCardId, newStatus: destinoColuna })
            .then(() => {
                console.log('Status da tarefa', this.draggedCardId, 'atualizado para', destinoColuna);
            })
            .catch((error) => {
